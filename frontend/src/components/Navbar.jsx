  ...
  return (
    <>
      ...
      >
        ...
        <div
          className={`absolute bottom-0 left-0 right-0 h-[1px] bg-gradient-to-r from-transparent via-[#C9A23F] to-transparent transition-opacity duration-500 ${
          ...
          <div className="flex items-center justify-between gap-2 sm:gap-4">
            ...
            <div className="hidden lg:flex items-center space-x-6 xl:space-x-8">
              {leftNav.map((link, index) => (
                ...
                >
                  ...
                  >
                    <span
                      className={`text-[11px] font-medium uppercase tracking-[0.2em] transition-all duration-500 ${
                        isActive(link.path)
                          ? 'text-[#C9A23F]'
                          : isScrolled
                    ...
                    <span
                      className={`absolute -bottom-2 left-0 h-[2px] bg-gradient-to-r from-[#C9A23F] to-[#D4AF37] transition-all duration-500 ${
                        isActive(link.path)
                          ? 'w-full opacity-100'
            ...
            >
              <Link to="/" className="group flex flex-col items-center">
                <h1
                  className={`text-sm sm:text-lg md:text-xl lg:text-2xl font-bold uppercase tracking-[0.08em] sm:tracking-[0.12em] lg:tracking-[0.15em] transition-all duration-500 relative ${
                    isScrolled
                ...
                <div
                  className={`text-[6px] sm:text-[7px] uppercase tracking-[0.2em] sm:tracking-[0.3em] transition-colors duration-500 hidden sm:block ${
                    isScrolled ? 'text-[#C9A23F]/70' : 'text-[#D4AF37]/80'
                  }`}
            ...
            <div className="hidden lg:flex items-center space-x-3 ml-4">
              {user ? (
                ...
                <>
                  <Link
                    to="/login"
                    className={`text-[10px] font-medium uppercase tracking-[0.15em] transition-all duration-500 hover:tracking-[0.2em] whitespace-nowrap ${
                      isScrolled
                        ? 'text-[#111111] hover:text-[#C9A23F]'
                        : 'text-white drop-shadow-[0_2px_8px_rgba(0,0,0,0.8)] hover:text-[#C9A23F]'
                    }`}
              ...
              {isMobileMenuOpen ? (
                <X
                  className={`w-5 h-5 sm:w-6 sm:h-6 transition-colors duration-300 ${
                    isScrolled ? 'text-[#111111]' : 'text-white drop-shadow-[0_2px_8px_rgba(0,0,0,0.8)]'
                  }`}
                />
              ) : (
                <Menu
                ...
                <div className="flex flex-col space-y-4 sm:space-y-6 mb-6 sm:mb-8">
                  {navLinks.map((link, index) => (
                    ...
                    >
                      ...
                      >
                        <span
                          className={`text-xs sm:text-sm font-medium uppercase tracking-[0.15em] sm:tracking-[0.2em] transition-all duration-300 ${
                            isActive(link.path)
                              ? 'text-[#C9A23F]'
                        ...
                        <span
                          className={`absolute -bottom-1 left-0 h-[2px] bg-gradient-to-r from-[#C9A23F] to-[#D4AF37] transition-all duration-300 ${
                            isActive(link.path)
                              ? 'w-full opacity-100'
                              : 'w-0 opacity-0 group-hover:w-full group-hover:opacity-100'
                          }`}
                  ...
                  {user ? (
                    ...
                    <>
                      <Link
                        to="/login"
                        onClick={() => setIsMobileMenuOpen(false)}
                        className="px-4 sm:px-6 py-2.5 sm:py-3 text-center bg-white border-2 border-[#C9A23F] text-[#C9A23F] hover:bg-[#C9A23F] hover:text-white transition-all duration-300"
                      >
